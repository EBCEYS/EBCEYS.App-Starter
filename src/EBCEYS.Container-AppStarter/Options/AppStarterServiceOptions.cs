...
internal class AppStarterServiceOptions(
    TimeSpan delayBeforeStart,
    string? execFile,
    string? execArgs,
    string? workingDir,
    TimeSpan requestPeriod,
    int requestReties,
    TimeSpan retriesDelay,
    bool restartOnConfUpdate)
{
    ...
    {
        var execFile = SupportedEnvironmentVariables.ExecFile.Value;
        var argsLine = SupportedEnvironmentVariables.ExecArgs.Value ?? string.Empty;
        if (string.IsNullOrWhiteSpace(execFile)) throw new Exception("No file to execute!");
        if (!File.Exists(execFile)) throw new FileNotFoundException("No execution file!", execFile);
        return new AppStarterServiceOptions(
